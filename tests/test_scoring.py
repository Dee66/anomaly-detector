...
from detector.scoring import (
    modified_z_scores,
    detect_anomalies_mod_z,
    windowed_modified_z_scores,
    recommend_reconciliation,
)


def test_modified_z_scores_happy_path():
    vals = [10, 12, 11, 13, 100]
    scores = modified_z_scores(vals)
    # The outlier at the end should have the largest score
...
def test_empty_input_returns_empty():
    ...
    assert detect_anomalies_mod_z([]) == []
    assert windowed_modified_z_scores([], window_size=3) == []


def test_constant_series_no_anomalies():
    vals = [5, 5, 5, 5, 5]
    scores = modified_z_scores(vals)
    assert all(math.isclose(s, 0.0) for s in scores)
    assert detect_anomalies_mod_z(vals) == [False] * len(vals)


def test_single_outlier_detected():
    vals = [1, 1, 1, 1, 50]
    # Use a slightly lower threshold to detect the large outlier when falling back
    # to std-based z-scores (MAD==0 case).
    flags = detect_anomalies_mod_z(vals, threshold=2.0)
    # Last value should be flagged
    assert flags[-1] is True
    recs = recommend_reconciliation(flags)
    assert isinstance(recs, dict)
    assert list(recs.keys()) == [len(vals) - 1]


def test_windowed_scoring_smoothed():
    vals = [1, 1, 100, 1, 1]
    w_scores = windowed_modified_z_scores(vals, window_size=3)
    # The central large spike should show up as the highest score
    assert len(w_scores) == len(vals)
    assert max(w_scores) == w_scores[2]
...
class TestStatisticalAnomalyDetector:
    ...
    def test_standard_zscore_calculation(self):
        ...
        detector = StatisticalAnomalyDetector(threshold=1.0, use_modified_zscore=False)
